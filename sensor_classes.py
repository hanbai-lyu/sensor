...
class Sensor(object):
    ...
    def print_measurements(self):
        ...
        if self.print_m:
            measurements_str = ["'%s': %.3g %s" % (channel, float(value), unit) for channel, value, unit in zip(self.channels, self.values, self.units)]
...
class Arduino_Sensor(Sensor):
    ...
    def read(self):
        """
        Expects serial input to be measurement values seperated by commas and ending with /r/n.
        """
        try:
            ser = serial.Serial(self.board_port, self.baud, timeout=10)
            values_raw = ser.readline()
            self.values = values_raw.decode().replace("\r\n", "").split(",")
            ser.close()
        ...
        for sensor in self.sensors:
            ...
            for filter_pass, channel, value in zip(mask, sensor.channels, sensor.values):
                if filter_pass:
                    ...
                    data_body["fields"][channel_name] = float(value)
